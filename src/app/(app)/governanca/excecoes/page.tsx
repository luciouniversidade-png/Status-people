        ...
        <div>
          ...
          <Table head={["#", "Regra / POP", "Motivo", "Referência", "Solicitante", "Situação", "Validade", ""]} empty="Nenhuma exceção neste filtro.">
            {rows.map(e => <tr key={e.id}><Td className="text-slate-500">{e.id}</Td><Td><div className="font-medium">{e.regra}</div>{e.pop && <div className="text-xs text-slate-500">{e.pop}</div>}{e.descricao && <div className="max-w-[260px] text-xs text-slate-600">{e.descricao}</div>}</Td><Td className="text-xs">{e.motivo}</Td><Td className="text-xs">{e.referencia ?? "—"}{e.unidade && <div className="text-slate-500">{e.unidade}</div>}</Td><Td className="text-xs">{e.solicitante}<div className="text-slate-500">{fmtDataHora(e.created_at)}</div></Td>
              ...
              <Td>{e.status === "SOLICITADA" && podeAprovar && e.solicitante_id !== s.id && <details className="relative"><summary className="cursor-pointer rounded-md bg-acao px-2.5 py-1 text-xs font-medium text-white">Decidir</summary><form action={decidirExcecao} className="mt-1 w-64 space-y-2 rounded-md border border-line bg-white p-3 text-xs shadow-lg"><input type="hidden" name="id" value={e.id} /><Field label="Validade até"><Input name="validadeAte" type="date" defaultValue={e.validade_ate ?? ""} /></Field><Field label="Nota"><Input name="nota" /></Field><div className="flex gap-1"><Btn small name="decisao" value="APROVADA">Aprovar</Btn><Btn small kind="ghost" name="decisao" value="REJEITADA">Rejeitar</Btn></div></form></details>}
                {(e.status === "EXPIRADA" || e.status === "APROVADA") && <details className="relative mt-1"><summary className="cursor-pointer rounded-md border border-line bg-white px-2.5 py-1 text-xs">Revisar</summary><form action={decidirExcecao} className="mt-1 w-64 space-y-2 rounded-md border border-line bg-white p-3 text-xs shadow-lg"><input type="hidden" name="id" value={e.id} /><input type="hidden" name="decisao" value="REVISADA" /><Field label="Conclusão da revisão"><Input name="nota" placeholder="Ex.: incorporada ao POP-003 v2" required /></Field><Btn small>Encerrar como revisada</Btn></form></details>}</Td></tr>)}
          </Table>
        </div>
